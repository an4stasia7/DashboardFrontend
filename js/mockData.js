...
(function (global) {
  ...
  /** Соответствие ролей с API именованным набору плиток в KPI_TILES_BY_ROLE */
  const ROLE_ALIASES = {
    User1: "Технический директор",
  };

  /**
   * Заглушка иерархии: с бэкенда позже придёт список пользователей,
   * чьи дашборды может открыть текущий пользователь (id, подпись вкладки, объект user для KPI).
   */
  /** По id пользователя из сессии (как придёт с бэкенда), не по логину входа */
